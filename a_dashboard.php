...
$sql = 'SELECT * FROM post_form WHERE verify in (0)';
...
<body>
  <nav class="navbar navbar-expand-md navbar-light bg-body border-bottom p-0 ps-5" id="navbarz">
    ...
    <div class="dropdown">
      ...
      <ul class="dropdown-menu" id="profile_drop">
        ...
        <li><a class="dropdown-item" href="logout.php?logoutt">Logout</a></li>
      </ul>
    </div>
  </nav>
  <div class="row" id="center_body">
      ...
      <nav class="sidebar bg-white pb-2">
        <div>
          <div class="list-group list-group-flush mx-3 mt-4">
            ...
            <a href="a_dashboard.php?dashboard" class="list-group-item list-group-item-action py-2 ripple   bg-danger text-white"><span>Verify Post</span></a>
    ...
    <div class="col">
          ...
          <div class="row">
            <div class="col-10 py-2">
              <a href="#" class="text-decoration-none text-dark">
                <?php echo $item['title'] ?> • <?php echo $item['auth_fname'] ?> <?php echo $item['auth_lname'] ?>
              </a>
            </div>
      ...
      <?php endforeach ?>
      <?php
      if (isset($_POST['verify']) && isset($_POST['pid'])) {
        $pid = $_POST['pid'];

        $log = $fname . " " . $lname . " Verified post #".$pid;
        $plog = "INSERT INTO `user_logs`(`fname`, `lname`, `role`, `action`) VALUES ('$fname','$lname','$role','$log')";
        mysqli_query($conn, $plog);

        $insert = "UPDATE post_form SET verify = 1 WHERE form_id = '$pid'";
        $updated = mysqli_query($conn, $insert);

        
        if ($updated) {
          echo "<script>window.location.href='a_dashboard.php';</script>";
        }
      }
      if (isset($_POST['reject']) && isset($_POST['pid'])) {
        $pid = $_POST['pid'];

        $log = $fname . " " . $lname . " Verified post #".$pid;
        $plog = "INSERT INTO `user_logs`(`fname`, `lname`, `role`, `action`) VALUES ('$fname','$lname','$role','$log')";
        mysqli_query($conn, $plog);
        $insert = "UPDATE post_form SET verify = 2 WHERE form_id = '$pid'";
        $updated = mysqli_query($conn, $insert);
        if ($updated) {
          echo "<script>window.location.href='a_dashboard.php';</script>";
        }
